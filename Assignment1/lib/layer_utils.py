...
class fc(object):
    ...
    def forward(self, feat):
        ...
        #############################################################################
        N = feat.shape[0]
        D_prod = np.prod(feat.shape[1:])
        output = np.dot(feat.reshape(N, D_prod),self.params[self.w_name]) + self.params[self.b_name]
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
    ...
    def backward(self, dprev):
        ...
        dfeat = (np.dot(dprev, self.params[self.w_name].T)).reshape(orig_shape)

        dw_name = np.dot(feat.reshape(N,D_prod).T, dprev)
        self.grads[self.w_name] = dw_name
                  
        db_name = np.sum(dprev, axis=0)
        self.grads[self.b_name] = db_name                  
...
class dropout(object):
    ...
    def backward(self, dprev):
        ...
        if(self.is_Training and self.dropped is not None):
...
class cross_entropy(object):
    ...
    def forward(self, feat, label):
        ...
        totalInput = feat.shape[0]
        loss = np.sum(-(np.log(scores[range(totalInput),label]))) / totalInput
    ...
    def backward(self):
        ...
        #############################################################################
        totalInput = self.label.shape[0]
        dLoss[range(totalInput),self.label] -= 1
        dLoss /= totalInput
