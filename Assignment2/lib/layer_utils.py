...
class LSTM(object):
    ...
    def step_forward(self, x, prev_h, prev_c):
        ...
        initial_a = x.dot(self.params[self.wx_name]) + prev_h.dot(self.params[self.wh_name]) + self.params[self.b_name]
        a_i, a_f, a_o, a_g = np.split(initial_a,4,axis=1)
        i = sigmoid(a_i)
        f = sigmoid(a_f)
        o = sigmoid(a_o)
        g = np.tanh(a_g)
        ...
        meta = [x, prev_h, next_h, next_c, prev_c, i, f, o, g]
    ...
    def step_backward(self, dnext_h, dnext_c, meta):
        ...
        #############################################################################
        x, prev_h, next_h, next_c, prev_c, i, f, o, g = meta
    ...
    def forward(self, x, h0):
        ...
        h[:,0,:], c[:,0,:], meta_i = self.step_forward(x[:,0,:], h0,0)        
        self.meta.append(meta_i)
        for i in range(1, x.shape[1]):
